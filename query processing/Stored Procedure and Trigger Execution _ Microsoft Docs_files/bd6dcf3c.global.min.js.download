(function() {
    ...
    var promise = location.hostname === "localhost" ? Promise.resolve(false) : null;
    function isExperimentEnabled(name) {
        if (!promise) {
            promise = fetchWithTimeout("/_api/experiment", {
                mode: "cors"
            }).then(function(res) {
        ...
        return promise.then(function(_a) {
            ...
            return features && features.indexOf(name) !== -1;
        });
    }
    var nextId$1 = 0;
    ...
    function loadUhf() {
        ...
        if (!isAzure && msDocs.settings.enableUHFPPE) {
            var path = removeLocaleFromPath(location.pathname);
            if (/^\/dotnet(?:\/|$)/i.test(path)) {
                promise = isExperimentEnabled("45745285-4192-41");
            } else if (/^\/visualstudio(?:\/|$)/i.test(path)) {
                promise = isExperimentEnabled("1934df9a-93fa-42");
            } else if (getMeta("force_uhf_ppe") === "true") {
    ...
    function loadAzureHeader() {
        var userLocale = msDocs.data.userLocale;
        var isRTL = rtlDictionary[userLocale];
        var azureHeaderLocale = isRTL ? "en-us" : userLocale;
    ...
    function renderLogin(container, processTokens) {
        reportLoginPrompt();
        container.innerHTML = "\n\t\t<h2>" + escape$1(loc.signIn) + "</h2>\n\t\t<p>" + escape$1(loc.signInAzure) + '</p>\n\t\t<button type="button" class="button-primary blue" data-bi-name="azure-auth-login">\n\t\t\t' + loc.signIn + "\n\t\t</button>";
    ...
    function reportLoginPrompt() {
        jsllReady.then(function(awa) {
            ...
            return awa.ct.captureContentPageAction({
                ...
                contentTags: (_a = {}, _a[contentTags.scenario] = "azure-auth", _a[contentTags.scenarioStep] = "login-prompt", 
                _a)
            });
        });
    }
    function reportLogin() {
        jsllReady.then(function(awa) {
            ...
            return awa.ct.captureContentPageAction({
                ...
                actionType: awa.actionType.CLICKLEFT,
                contentTags: (_a = {}, _a[contentTags.scenario] = "azure-auth", _a[contentTags.scenarioStep] = "login", 
                _a)
            });
        });
    }
    function reportAuthorized() {
        jsllReady.then(function(awa) {
            ...
            return awa.ct.captureContentPageAction({
                ...
                actionType: awa.actionType.OTHER,
                contentTags: (_a = {}, _a[contentTags.scenario] = "azure-auth", _a[contentTags.scenarioStep] = "authorized", 
                _a)
            });
        });
    }
    var DockPanel = function() {
        ...
        Object.defineProperty(DockPanel.prototype, "component", {
            ...
            set: function(value) {
                ...
                value.commands.forEach(function(c) {
                    return _this.createCommandElement(c);
                });
            },
            enumerable: true,
            configurable: true
    ...
    function showCloudShell(isPowerShell) {
        ...
        if (current && current instanceof CloudShell && current.isPowerShell === isPowerShell) {
            ...
            dockPanel.component = new CloudShell(isPowerShell, function() {
                return dockPanel.close();
            });
            setTimeout(function() {
                return dockPanel.maximize();
            });
        }
        return Promise.resolve();
    }
    function createForTutorial(isPowerShell) {
        ...
        panel.classList.add("tutorial-cloud-shell");
    ...
    registerInteractiveType({
        ...
        activateCodeBlock: function() {
            return showCloudShell(false);
        },
        createForTutorial: function() {
            return createForTutorial(false);
        }
    });
    registerInteractiveType({
        ...
        activateButtonConfig: activateButtonConfig,
        activateCodeBlock: function() {
            return showCloudShell(true);
        },
        createForTutorial: function() {
            return createForTutorial(true);
        }
    });
    ...
    registerInteractiveType({
        ...
        activateCodeBlock: function(codeBlock) {
            return activateCodeBlock(codeBlock, "csharp");
        },
        createForTutorial: function() {
            ...
            runButton.onclick = function() {
                ...
                dotnetOnline.run().catch(function() {}).then(function() {
    ...
    function activateCodeBlock(codeBlock, language) {
        var instance = activatedCodeBlocks.find(function(x) {
            return x.element.parentElement === codeBlock.parentElement;
        });
        if (instance) {
            return instance.run();
        }
        ...
        var targetHeight = Math.min(height + 123, Math.floor(viewportHeight * .85));
    ...
    var nextUnit = {
        ...
        set value(newValue) {
            if (location.hostname === "localhost" && newValue.url !== getLearnHomeUrl()) {
                newValue.url = "/" + msDocs.data.userLocale + newValue.url;
            }
            next = newValue;
        }
    };
    ...
    var CombinedProgressService = function() {
        function CombinedProgressService(user, unitUid, progressTransferred) {
            ...
            this.local = new LocalProgressService(localStorage, "section_" + location.pathname.replace(/[^a-zA-Z\d\s]+/g, "_"));
        }
    ...
    var RemoteProgressService = function() {
        ...
        RemoteProgressService.prototype.processProgressData = function(data) {
            ...
            if (data.next) {
                nextUnit.value = data.next;
            }
            return data.items.reduce(function(progress, step) {
                if (step.type === "quiz") {
                    progress.quizComplete = step.isCompleted;
                } else {
    ...
    function tutorialPage() {
        contentLoaded.then(function() {
            ...
            var quizUid = getMeta("challenge_uid");
            progressService = new CombinedProgressService(user, unitUid, renderToc);
            processQueryString();
            ...
            if (unitUid && quizUid) {
                renderQuizStep(unitUid, quizUid);
            }
        });
    }
    ...
    function showStep(step) {
        ...
        if (steps.item(step).getAttribute("tutorial-step-type") !== "quiz") {
    ...
    function ensureInteractive() {
        ...
        var interactiveComponent = interactiveTypes[msDocs.data.tutorialInteractiveType].createForTutorial();
    ...
    function renderToc(progress) {
        ...
        document.querySelector(".toc").innerHTML = '\n\t\t<ol class="tutorial-toc">\n\t\t\t' + steps.map(function(_a) {
            ...
            return '\n\t\t\t<li class="' + (progress[index] || progress.quizComplete && type === "quiz" ? "completed-step" : "") + " " + (index === activeStep ? "active-step" : "") + '">\n\t\t\t\t<a class="tutorial-nav-behavior" href="?tutorial-step=' + index + '">\n\t\t\t\t\t' + checkMark + "\n\t\t\t\t\t<span>" + title + "</span>\n\t\t\t\t</a>\n\t\t\t</li>";
        }).join("") + "\n\t\t</ol>";
    }
    function renderQuizStep(unitUid, quizUid) {
        ...
        user.whenAuthenticated().then(function() {
            ...
            if (progress.quizComplete) {
                renderQuizComplete(form);
                setQuizNextStepButton();
                return;
            }
            getQuiz(quizUid).then(function(quiz) {
                return renderQuiz(form, quiz, quizUid);
            });
        });
    }
    function setQuizNextStepButton() {
        var _a = nextUnit.value || defaultNextUnit, url = _a.url, title = _a.title;
        var navBlock = document.querySelector('.tutorial-step[tutorial-step-type="quiz"] .tutorial-nav');
        var existingNextButton = navBlock.querySelector(".tutorial-nav-button-next");
        if (existingNextButton) {
            navBlock.removeChild(existingNextButton);
        }
        navBlock.insertAdjacentHTML("beforeend", '\n\t\t<a class="tutorial-nav-button tutorial-nav-button-next x-hidden-focus" href="' + url + '">\n\t\t\t' + title + '\n\t\t\t<span class="docon docon-arrow-right" aria-hidden="true"></span>\n\t\t</a>');
    }
    ...
    function renderQuestion(container, bus, question) {
        ...
        divBlock.classList.add("learn-question");
        ...
        question.choices.forEach(function(choice) {
            ...
            var choiceDescSection = document.createElement("div");
            choiceDescSection.classList.add("choice-desc-group");
            choiceDescSection.innerHTML = choice.content;
            var choiceExplanation = document.createElement("div");
            choiceExplanation.classList.add("explanation");
            choiceExplanation.hidden = true;
            choiceDescSection.appendChild(choiceExplanation);
            choiceLabel.appendChild(choiceDescSection);
            choiceLabel.onclick = function(event) {
                resetChoiceStatus(choiceBlock);
                resetQuestionStatus(divBlock);
                selectedChoiceId = parseInt(choiceInput.value);
                divBlock.classList.add("answered");
            };
            bus.subscribe(QuizCheckAnswer, function(_a) {
                var gradedQuestionData = _a.gradedQuestionData;
                choiceExplanation.hidden = true;
                var gradedQuestion = gradedQuestionData.filter(function(item) {
                    return item.id === question.id;
                })[0];
                var gradedChoice = gradedQuestion.choices.filter(function(item) {
                    return item.id === choice.id;
                })[0];
                if (gradedChoice !== undefined) {
                    if (!gradedChoice.isCorrect) {
                        choiceExplanation.hidden = false;
                        if (gradedChoice.explanation !== undefined && gradedChoice.explanation !== null) {
                            choiceExplanation.innerHTML = gradedChoice.explanation;
                        }
                    }
                }
            });
        ...
        bus.subscribe(QuizCheckAnswer, function(_a) {
            ...
            if (!gradedQuestion.isCorrect) {
                divBlock.classList.add("incorrect");
                ...
                divBlock.classList.add("correct");
                spanChoice.textContent = "";
                spanChoice.classList.add("docon");
    ...
    function resetQuestionStatus(divBlock) {
        divBlock.classList.remove("answered");
        divBlock.classList.remove("correct");
        divBlock.classList.remove("incorrect");
    }
    var CheckQuizErrorResponse = function() {
        function CheckQuizErrorResponse(responseStatus) {
            this.responseStatus = responseStatus;
        }
        return CheckQuizErrorResponse;
    }();
    ...
    function renderQuiz(container, quiz, id) {
        ...
        container.onsubmit = function(event) {
            ...
            submitQuizChallenge(checkChallengeButton, errorLabel, bus);
            event.preventDefault();
        };
        return divBlock;
    }
    function renderCheckChallengeButton(container) {
        ...
        checkButton.classList.add("authenticated");
        checkButton.setAttribute("type", "submit");
        checkButton.setAttribute("data-bi-name", "learn-quiz-validate");
        checkButton.textContent = loc.checkAnswers;
        container.appendChild(checkButton);
        var signInCheckButton = document.createElement("button");
        signInCheckButton.classList.add("button-primary");
        signInCheckButton.classList.add("blue");
        signInCheckButton.classList.add("not-authenticated");
        signInCheckButton.classList.add(signInClassName);
        signInCheckButton.setAttribute("type", "button");
        signInCheckButton.textContent = "Sign in to check challange";
        container.appendChild(signInCheckButton);
        return checkButton;
    }
    function submitQuizChallenge(checkButton, errorLabel, bus) {
        checkButton.classList.add("busy");
        var selectedAnswerData = [];
        bus.publish(new QuizRetrieveSelectedAnswerData(selectedAnswerData));
        bus.subscribe(CheckQuizErrorResponse, function(_a) {
            errorLabel.textContent = loc.quizCouldNotGrade;
        });
        checkQuiz(unitId, quizId, selectedAnswerData, bus).then(function(data) {
            if (data !== null) {
                if (data.next) {
                    nextUnit.value = data.next;
                }
                if (data.isUpdated) {
                    if (data.achievements) {
                        currentModuleName = data.achievements[0].sourceItem.title;
                        achievementUrl = data.achievements[0].imageUrl;
                        displayGreetingDialog(false, data.achievements[0].sourceItem.url);
                        reportLearnEvent("learn-achievement-earned", data.achievements[0].uid);
                        reportLearnEvent("learn-module-completed", data.achievements[0].sourceItem.uid);
                    } else {
                        var gradedQuestionsCorrect = selectedAnswerData.map(function(item) {
                            return {
                                id: item.id,
                                isCorrect: true,
                                choices: [ {
    ...
    function renderQuizComplete(quizForm) {
        var quizStepHeading = document.querySelector('.tutorial-step[tutorial-step-type="quiz"] .tutorial-step-heading');
        quizStepHeading.textContent = loc.quizComplete;
        quizStepHeading.insertAdjacentHTML("beforeend", '<span class="docon docon-check quiz-complete-icon" aria-hidden="true"></span>');
        if (!nextUnit.value || nextUnit.value === defaultNextUnit) {
            quizForm.innerHTML = "";
            return;
        }
        quizForm.innerHTML = '\n\t\t<div class="complete-section">\n\t\t\t<p>' + escape$1(loc.quizCompletedText.replace("{tutorial}", tutorialName).replace("{nextUnitName}", nextUnit.value.title)) + "</p>\n\t\t</div>";
    }
    function reportLearnEvent(type, uid) {
        jsllReady.then(function(awa) {
            awa.ct.captureContentPageAction({
                behavior: awa.behavior.OTHER,
                actionType: awa.actionType.OTHER,
    ...
    function reportChallengeResult(gradedQuestions, bus, passed) {
        var gradedQuestionForBi = gradedQuestions.map(function(item) {
            return {
                id: item.id,
    ...
    function displayGreetingDialog(isUnit, moduleUrl) {
        if (isUnit === void 0) {
            isUnit = true;
        }
        if (moduleUrl === void 0) {
        ...
        closeButton.addEventListener("click", function() {
            dialog.hide();
            renderQuizComplete(formContainer);
            setQuizNextStepButton();
        });
        ...
        if (isUnit) {
            if (nextUnit.value === defaultNextUnit) {
                continueText.textContent = loc.quizFinishSectionContinueMoreTutorials;
            } else {
                continueText.textContent = loc.quizFinishSection.replace("{nextUnitName}", nextUnit.value.title);
            }
        } else {
            continueText.textContent = loc.quizFinishSectionModule.replace("{moduleName}", currentModuleName);
        }
        ...
        if (isUnit) {
            ...
            greetImg.classList.add("achieve-img");
            greetImg.setAttribute("src", achievementUrl);
            greetImg.setAttribute("aria-hidden", "true");
            dialogBlock.appendChild(greetImg);
        }
        var dialog = new Dialog();
        var nextAnchor = document.createElement("a");
        nextAnchor.setAttribute("href", nextUnit.value ? nextUnit.value.url : getLearnHomeUrl());
        nextAnchor.classList.add("button-primary");
        nextAnchor.classList.add("next-button");
        ...
        if (nextUnit.value) {
            if (nextUnit.value === defaultNextUnit) {
                nextAnchor.textContent = loc.moreTutorials;
            } else {
                nextAnchor.textContent = nextUnit.value.title;
            }
        } else {
            nextAnchor.textContent = loc.moreTutorials;
        }
        ...
        if (!isUnit) {
            ...
            shareSection.setAttribute("data-bi-name", "share-links");
            shareSection.classList.add("share-links");
            shareSection.innerHTML = '\n\t\t<a class="share-twitter" data-bi-name="social twitter"><span class="docon docon-brand-twitter"></span></a>\n\t\t<a class="share-facebook" data-bi-name="social facebook"><span class="docon docon-brand-facebook"></span></a>\n\t\t<a class="share-linkedin" data-bi-name="social linkedin"><span class="docon docon-brand-linkedin"></span></a>';
            dialogBlock.appendChild(shareSection);
            var shareMessage = loc.shareAchievementMessage.replace("{moduleName}", currentModuleName);
            initSharingLinks(dialogBlock, location.origin + "/" + msDocs.data.userLocale + moduleUrl, shareMessage);
        }
        dialog.show(dialogBlock, {
            modal: true,
    ...
    function getModuleListProgress() {
        var url = getServerUrl() + "/progress/modules/";
        var init = {
            method: "GET",
            credentials: "include"
        };
    ...
    function getQuiz(quizId) {
        ...
        var url = getServerUrl() + "/quizzes/" + quizId + "?" + toQueryString(query);
        var init = {
            method: "GET",
            headers: {
                quizId: quizId
    ...
    function checkQuiz(unitId, quizId, selectedAnswerData, bus) {
        var query = {
            branch: branch
        };
        var url = getServerUrl() + "/progress/units/" + unitId + "/steps/" + quizId + "?" + toQueryString(query);
        var init = {
            method: "PUT",
            credentials: "include",
        ...
        return fetchWithTimeout(createRequest(url, init)).then(function(response) {
            ...
            if (response.ok) {
                ...
                bus.publish(new CheckQuizErrorResponse(response.status));
                return null;
            }
        });
    }
    ...
    function getModules() {
        ...
        var url = getServerUrl() + "/hierarchy/modules?" + toQueryString(query);
        var init = {
            method: "GET"
        };
        return fetchWithTimeout(createRequest(url, init)).then(function(response) {
    ...
    function learningPathPage() {
        getModules().then(function(modules) {
            ...
            for (var _i = 0, modules_1 = modules; _i < modules_1.length; _i++) {
                var _a = modules_1[_i], uid = _a.uid, units = _a.units, duration = _a.duration;
                if (document.getElementById(uid)) {
                    var moduleDetails = document.getElementById(uid).querySelector(".module-details");
                    var totalUnits = units.length;
                    var totalFormattedDuration = convertMinsToHrsMins(duration);
                    var uni = totalUnits > 1 ? loc.units : loc.unit;
                    moduleDetails.textContent = "Module - " + totalUnits + " " + uni + " - " + totalFormattedDuration;
                    uidsOnPage["" + uid] = true;
                }
            }
            user.whenAuthenticated().then(function() {
                getModuleListProgress().then(function(moduleListProgress) {
                    var list = moduleListProgress.filter(function(moduleProgress) {
                        return moduleProgress.uid in uidsOnPage && moduleProgress.isCompleted;
                    });
                    for (var _i = 0, list_1 = list; _i < list_1.length; _i++) {
                        var uid = list_1[_i].uid;
                        var moduleDetails = document.getElementById(uid).querySelector(".module-details");
                        moduleDetails.innerHTML = '<span aria-label="' + loc.complete + '" class="docon docon-check"></span>';
                    }
                });
            });
        });
    ...
    function modulePage() {
        user.whenAuthenticated().then(displayProgress);
    }
    function displayProgress() {
        var progress = getModuleProgress(getMeta("uid"));
    ...
    function deleteProfileConfirmationTemplate() {
        return '\n\t\t<section class="dialog-message">\n\t\t\t<h1>' + loc.areYouSure + "</h1>\n\t\t\t<p>" + loc.areYouSureDescription + '</p>\n\t\t\t<div class="dialog-actions">\n\t\t\t\t<button data-bi-name="profile-cancel-delete" class="secondary-action button-border">' + loc.close + '</button>\n\t\t\t\t<button data-bi-name="profile-confirm-delete" class="warning button-primary">' + loc.delete + "</button>\n\t\t\t</div>\n\t\t</section>\n\t";
    }
    ...
    function confirmDeleteUserProgressHandler(event, dialog) {
        ...
        viewElement.querySelector(".secondary-action").addEventListener("click", dialog.hide.bind(dialog));
        viewElement.querySelector(".button-primary").addEventListener("click", function(event) {
            ...
            $.ajax({
                ...
                if (data.count > 0) {
                    ...
                    data.results.forEach(function(r) {
                        ...
                        html.push("<div><a data-bi-name='searchItem." + index_1 + "' href='" + r.url + "' class='title'>" + escape$1(r.title) + "</a>");
    ...
    var themeMap = {
        light: {
            color: "hsla(0, 0%, 0%, 1)",
            "color-subtle": "hsla(0, 0%, 20%, 1)",
            link: "hsla(216, 100%, 39%, 1)",
            "background-color": "hsla(0, 0%, 100%, 1)",
            "border-color": "hsla(0, 0%, 88%, 1)",
            "footer-background": "hsla(0, 0%, 95%, 1)",
            "footer-link": "hsla(0, 0%, 38%, 1)",
            "footer-link-active-hover": "hsla(0, 0%, 0%, 0.8)",
            "breadcrumb-background": "hsla(200, 13%, 95%, 1)"
        },
        night: {
            color: "hsla(0, 0%, 80%, 1)",
            "color-subtle": "hsla(0, 0%, 51%, 1)",
            link: "hsla(200, 100%, 48%, 1)",
            "background-color": "hsla(0, 0%, 10%, 1)",
            "border-color": "hsla(0, 0%, 20%, 1)",
            "footer-background": "hsla(0, 0%, 18%, 1)",
            "footer-link": "hsla(200, 100%, 48%, 1)",
            "footer-link-active-hover": "hsla(207, 100%, 42%, 1)",
    ...
    function createToc() {
        ...
        var filterToc = function(inputField) {
            ...
            var scope = getMeta("scope") !== undefined ? escape$1(getMeta("scope")) : "";
            ...
            if (val && val.length) {
                ...
                if (resultIsEmpty_1) {
                    var url = "/search/index?search=" + encodeURIComponent(val) + "&scope=" + scope;
                    if (brand === "mooncake") {
                    ...
                    $emptyHolder.html(loc.noResultsToc.replace("{term}", val).replace("{filter-text}", val).replace("{scope}", scope).replace("{url}", url));
                }
            } else if ($savedToc) {
